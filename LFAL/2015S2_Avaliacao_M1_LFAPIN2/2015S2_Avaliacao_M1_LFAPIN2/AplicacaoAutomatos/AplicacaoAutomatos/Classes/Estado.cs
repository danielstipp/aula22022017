...
namespace AplicacaoAutomatos.Classes
{
    ...
    public class Estado
    {
        ...
        public Transicao BuscaTransicao(char simbolo)
        {
            ...
            List<Transicao> transicao = new List<Transicao>();

            foreach (var item in Transicoes)
            {
                if (item.Simbolo.Contains(simbolo))
                {
                    transicao.Add(item);
                }
                else if (item.Simbolo.Contains('&'))
                {
                    Transicao t = item.Destino.Transicoes.First();
                    while (t != null)
                    {
                        if (t.Simbolo.Contains(simbolo)) { transicao.Add(t); break; }
                        else { t = t.Destino.Transicoes.First(); }
                    }
                }
            }
